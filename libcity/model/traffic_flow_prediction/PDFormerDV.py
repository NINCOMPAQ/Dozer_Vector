...
from DOZER_VECTOR.DOZER_VECTOR import VFPE, dozer_attention
...
class STSelfAttention(nn.Module):
    def __init__(
        ...
        self.doz_attn = dozer_attention(stride, local_window)

        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None, dozer=True):
        B, T, N, D = x.shape
        #print('data shape is:', x.shape)
        device = x.device
        geo_mask = geo_mask.to(device) if geo_mask is not None else None
        sem_mask = sem_mask.to(device) if sem_mask is not None else None
        ...
        if dozer:
            t_x = self.doz_attn(t_q, t_k, t_v, [B,T,N,D])
...
class PDFormerDV(AbstractTrafficStateModel):
    ...
    def forward(self, batchx, batchy, lap_mx=None, VF=None):
        ...
        VF = VF.to(device)
        self.pattern_keys = self.pattern_keys.to(device)
        T =  x.shape[1]
        x_pattern_list = []
